    ...
    <div class="containeer">
            ...
            <div class="col-md-10">
                <div class="card brode-0 shadow-lg my-4">
                    <div class="card-header bg-dark">
                        <h3 class="text-white"> create seats</h3>
                    </div>
                    <div class="card-body">
        ...
        <form action="{{ route('seats.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div class="mb-4">
                <label for="seat-code" class="form-label h5">Seat Code</label>
                <select id="seat-code" name="seatCode">
                    <option value="A12" {{ old('seatCode') == 'A12' ? 'selected' : '' }}>A12</option>
                    <option value="B5" {{ old('seatCode') == 'B5' ? 'selected' : '' }}>B5</option>
                    <option value="C15" {{ old('seatCode') == 'C15' ? 'selected' : '' }}>C15</option>
                    <option value="D7" {{ old('seatCode') == 'D7' ? 'selected' : '' }}>D7</option>
                </select>
                @error('seatCode')
                    <div class="text-danger my-2">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="radio" name="isBooked" value="1" {{ old('isBooked') == '1' ? 'checked' : '' }}> Booked
                <input type="radio" name="isBooked" value="0" {{ old('isBooked') == '0' ? 'checked' : '' }}> Not Booked
                @error('isBooked')
                    <div class="text-danger my-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="radio" name="under_maintainance" value="1" {{ old('under_maintainance') == '1' ? 'checked' : '' }}> True
                <input type="radio" name="under_maintainance" value="0" {{ old('under_maintainance') == '0' ? 'checked' : '' }}> False
                @error('under_maintainance')
                    <div class="text-danger my-2">{{ $message }}</div>
                @enderror
            </div>
